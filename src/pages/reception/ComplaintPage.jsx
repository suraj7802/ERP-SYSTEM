 ...
 * Image 11: Complaint List — SL, COMPLAINT TYPE, COMPLAINANT NAME, MOBILE NO, DATE, DATE OF SOLUTION, ASSIGN TO, ACTION
 * Image 12: Add Complaint — Type*, Assign To*, Complainant Name*, Mobile, Note, Date*, Document File
 */
...
function AddComplaintForm({ onSave, onCancel }) {
  const [form, setForm] = useState({ type: "", assignTo: "", name: "", mobile: "", note: "", date: "2026-05-27", docFile: "" });
  ...
  return (
    <div className="max-w-2xl">
      ...
      <Field label="Assign To" req>
        <select className={sel} value={form.assignTo} onChange={set("assignTo")}>
          <option value="">Select</option>
          {ASSIGN_TO_LIST.map(a => <option key={a}>{a}</option>)}
        </select>
      </Field>
      <Field label="Complainant Name" req>
        <input className={inp} value={form.name} onChange={set("name")} />
      </Field>
      <Field label="Complainant Mobile No">
        <input className={inp} value={form.mobile} onChange={set("mobile")} />
      </Field>
      <Field label="Note">
        <textarea className={inp} rows={4} value={form.note} onChange={set("note")} />
      </Field>
      ...
      <Field label="Document File">
        <div className="flex gap-2 items-center">
          <input readOnly className={inp + " flex-1 cursor-pointer"} value={form.docFile || ""} placeholder="No file chosen" />
          <button className="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-medium rounded-lg whitespace-nowrap transition-colors">Select file</button>
        </div>
      </Field>
      <div className="flex justify-center gap-3 mt-4">
        ...
        <button
          onClick={() => { if (!form.type || !form.assignTo || !form.name) return; onSave(form); }}
...
const TABLE_COLS = ["SL","COMPLAINT TYPE","COMPLAINANT NAME","MOBILE NO","DATE","DATE OF SOLUTION","ASSIGN TO","ACTION"];
...
export default function ComplaintPage() {
  ...
  const handleSave = form => {
    setComplaints(p => [...p, { ...form, id: Date.now(), dateSolution: "" }]);
    setTab("list");
  };
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-5">
        ...
        <h1 className="text-[15px] font-bold text-slate-800">Complaint</h1>
      </div>

      <div className="bg-white rounded-xl border border-slate-100 shadow-sm">
        <div className="px-5 border-b border-slate-100 flex gap-0">
          {[{k:"list",l:"Complaint List",i:"☰"},{k:"add",l:"Add Complaint",i:"✎"}].map(t => (
        ...
        <div className="p-5">
          {tab === "list" ? (
              ...
              <div className="border border-slate-100 rounded-xl overflow-x-auto">
                <table className="w-full text-sm min-w-[900px]">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {rows.length === 0 ? (
                      <tr><td colSpan={TABLE_COLS.length} className="px-4 py-10 text-center text-xs text-slate-400">No data available in table</td></tr>
                    ) : rows.map((r, i) => (
                      <tr key={r.id} className="hover:bg-slate-50/60">
                        ...
                        <td className="px-4 py-3 text-slate-500">{r.dateSolution || "—"}</td>
                        <td className="px-4 py-3 text-slate-600">{r.assignTo}</td>
                        <td className="px-4 py-3">
                          <div className="flex gap-1">
                            ...
                            <button onClick={() => setComplaints(p => p.filter(x => x.id !== r.id))} className="p-1.5 rounded bg-red-50 hover:bg-red-100 text-red-500"><Trash2 size={13}/></button>
                          </div>
                        </td>
                      </tr>
                    ))}
            ...
            <AddComplaintForm onSave={handleSave} onCancel={() => setTab("list")} />
          )}
        </div>
      </div>
    </div>
