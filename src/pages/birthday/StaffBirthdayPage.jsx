...
export default function StaffBirthdayPage() {
  const [filtered, setFiltered] = useState(false);
  const [date] = useState("2026/05/27 - 2026/05/27");
  return (
    <div>
      <PageTitle title="Staff Date Of Birth List"/>
      ...
      {filtered && <Card2><div className="p-8 text-center text-slate-400 text-sm">No staff birthdays for the selected date range.</div></Card2>}
    </div>
  );
}
