/**
 * AdmitCardTemplatePage.jsx — Image 2 (Add Admit Card) & Image 3 (Admit Card List)
 * URL: /cards/admit-template
 * List columns: SL, ADMIT CARD NAME, PAGE LAYOUT, BACKGROUND IMAGE, CREATED AT, ACTION
 * Add form: Admit Card Name*, Page Layout*, QR Code Text*, User Photo Style*, Layout Spacing*, Signature/Logo/Background, Certificate Content*
 */
import { useState } from "react";
import { Copy, FileText, FileDown, Printer, Search, Edit2, Trash2, Plus, AlignJustify, ChevronLeft, ChevronRight } from "lucide-react";
...
const TOOLBAR = ["⚡▾","Arial▾","14▾","T▾","B","I","U","abc","A","▾","≡","≡","☰","🔗","⊞","✕","↩","</>"];
...
function AddAdmitCardForm({ onSave, onCancel }) {
  const [form, setForm] = useState({ name:"", width:"", height:"", qrCode:"", photoStyle:"Square", photoSize:"", topSpace:"", bottomSpace:"", rightSpace:"", leftSpace:"" });
  ...
  return (
    <div className="max-w-3xl">
      <div className="grid grid-cols-3 items-center gap-4 mb-4">
        <label className="text-sm text-slate-600 font-medium text-right">Admit Card Name <span className="text-red-500">*</span></label>
      ...
      <div className="grid grid-cols-3 items-center gap-4 mb-4">
        <label className="text-sm text-slate-600 font-medium text-right">QR Code Text <span className="text-red-500">*</span></label>
        <div className="col-span-2">
          <select className={sel} value={form.qrCode} onChange={set("qrCode")}>
            <option value="">Select</option>
            <option>Student ID</option><option>Roll Number</option><option>Register No</option><option>Enroll No</option>
          </select>
        </div>
      </div>
      <div className="grid grid-cols-3 items-center gap-4 mb-4">
      ...
      <div className="grid grid-cols-3 items-start gap-4 mb-4">
        ...
        <div className="col-span-2">
          <div className="border border-slate-200 rounded-lg overflow-hidden">
            ...
            <textarea className="w-full px-3 py-2 text-sm focus:outline-none min-h-[140px] resize-y" value={content} onChange={e=>setContent(e.target.value)} placeholder="Enter admit card content..."/>
          </div>
        </div>
      </div>
      <div className="flex justify-center gap-3 mt-6">
        ...
        <button onClick={()=>{ if(!form.name) return; onSave(form,content); }} className="flex items-center gap-2 px-6 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-lg shadow-sm">
          <Plus size={13}/> Save
        </button>
      </div>
    </div>
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-5">
        ...
        <h1 className="text-[15px] font-bold text-slate-800">Admit Card Templete</h1>
      </div>
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm">
        <div className="px-5 border-b border-slate-100 flex gap-0">
          {[{k:"list",l:"Admit Card List",i:"☰"},{k:"add",l:"Add Admit Card",i:"✎"}].map(t=>(
            ...
            <>
              <div className="flex items-center justify-between mb-3">
                <div className="flex gap-1">
                  {[Copy,FileText,FileDown,FileDown,Printer].map((Icon,i)=><button key={i} className="p-1.5 rounded hover:bg-slate-100 text-slate-500"><Icon size={13}/></button>)}
                  <button className="p-1.5 rounded hover:bg-slate-100 text-slate-500 text-xs font-bold">⊞</button>
                </div>
                <div className="relative">
                  <Search size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400"/>
                  <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search..." className="pl-8 pr-4 py-1.5 text-sm border border-slate-200 rounded-lg focus:outline-none focus:border-indigo-400 w-48"/>
                </div>
              </div>
              <div className="border border-slate-100 rounded-xl overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="bg-slate-50">
                    {["SL","ADMIT CARD NAME","PAGE LAYOUT","BACKGROUND IMAGE","CREATED AT","ACTION"].map(h=>(
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {rows.length===0 ? (
                      <tr><td colSpan={6} className="px-4 py-10 text-center text-xs text-slate-400">No data available in table</td></tr>
                    ) : rows.map((r,i)=>(
                      <tr key={r.id} className="hover:bg-slate-50/60">
                        ...
                        <td className="px-4 py-3 text-slate-600">{r.pageLayout}</td>
                        <td className="px-4 py-3">{r.hasBg ? <div className="w-12 h-9 bg-blue-100 border border-blue-300 rounded flex items-center justify-center text-blue-400 text-xl">🖼</div> : "—"}</td>
              ...
              <div className="flex items-center justify-between mt-3 text-xs text-slate-500">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button className="p-1 rounded hover:bg-slate-100"><ChevronLeft size={14}/></button>
                  <button className="p-1 rounded hover:bg-slate-100"><ChevronRight size={14}/></button>
                </div>
              </div>
            </>
          ) : (
            <AddAdmitCardForm
              onSave={(form,content)=>{setTemplates(p=>[...p,{id:Date.now(),name:form.name,pageLayout:`${form.width}mm x ${form.height}mm`,hasBg:false,createdAt:"27.May.2026"}]);setTab("list");}}
